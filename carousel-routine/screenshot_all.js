...
(async () => {
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    try {
      await page.screenshot({ path: outPath, clip: {x:0, y:0, width:1080, height:1350}, timeout: 60000 });
  ...
  const infHTML = fs.readFileSync(path.join(__dirname, '../linkedin-infographic.html'), 'utf8');
