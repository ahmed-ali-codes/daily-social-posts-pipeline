/**
 * schedule_company_infographic.cjs
 * ===========================
 * Connects to the persistent Chrome session and schedules JUST the {{BRAND_SHORT_NAME}}
 * infographic post (which failed previously due to a scoping bug).
 */
...
(async () => {
  ...
  // Find the infographic post
  const post = ecoPosts.find(p => p.type === 'infographic');
  if (!post) {
    console.error("No infographic post found for {{BRAND_SHORT_NAME}}.");
    process.exit(1);
  }

  if (post.infographic_png && fs.existsSync(post.infographic_png)) {
    post.assetPath = path.resolve(post.infographic_png);
  } else {
    console.error("Infographic PNG file not found.");
    process.exit(1);
  }
  ...
  try {
    ...
    // If we can't connect, let's just launch using the persistent dir again
    const userDataDir = path.resolve(os.homedir(), '.agent-browser-data');
    console.log(`Connection failed, launching persistent Chrome directly...`);
    browser = await puppeteer.launch({
      executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',
      headless: false,
      userDataDir: userDataDir,
      args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-gpu', '--window-size=1280,1200', '--lang=en-US'],
  ...
  console.log(`\n──────────────────────────────────────────────────`);
  console.log(`Scheduling {{BRAND_SHORT_NAME}} Post ${post.id} | ${post.type} | ${post.slot}`);
  console.log(`──────────────────────────────────────────────────`);
  
  await page.goto(ECOTRUSTIA_PAGE, { waitUntil: 'domcontentloaded', timeout: 15000 });
  await sleep(4000);
  ...
  // BUG FIX: condition evaluated outside the browser context
  let clickedAdd = await clickNativelyShadow(page, (root) => {
    const btns = Array.from(root.querySelectorAll('button'));
    return btns.find(b => (b.ariaLabel || '').includes('Add media') || (b.ariaLabel || '').includes('image'));
  });
  ...
  console.log(`✓ Post ${post.id} scheduled successfully for ${post.date} at ${post.slot}`);
