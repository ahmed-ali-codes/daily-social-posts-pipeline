...
description: Generates 5 LinkedIn posts modeled on @{{BRAND_SHORT_NAME_LOWER}}'s OWN top-performing analytics — founder-psychology contrarian, loaded poll, AI-news-with-implications, story carousel, and data-visual-with-hook. Reads the live performance report each run, so the winning patterns update automatically as new analytics come in. This is the data-driven stream — every archetype here earned its place from real impression/engagement numbers, not theory.
allowed-tools: WebFetch, WebSearch, Bash, Read, Write
---

# LinkedIn Performance Engine

You generate the **5 report-driven posts** for the daily {{BRAND_NAME}} batch. Unlike the other engines (which are built on general best-practice), every archetype here is reverse-engineered from **@{{BRAND_SHORT_NAME_LOWER}}'s actual LinkedIn analytics**. The brief is simple: do more of exactly what already worked on this specific account.
...
cat ./{{BRAND_SHORT_NAME_LOWER}}_linkedin_content_report.md
```
...
[CTA — "Follow @{{BRAND_SHORT_NAME_LOWER}} for more" OR "Repost to help a founder who needs to hear this."]
```
...
[CTA — "Follow @{{BRAND_SHORT_NAME_LOWER}} for more" or save-prompt.]
```
...
**Voice check:**
- [ ] Declarative @{{BRAND_SHORT_NAME_LOWER}} voice, matches the cadence of the report's WINNING_HOOKS.
...
Source report: {{BRAND_SHORT_NAME_LOWER}}_linkedin_content_report.md (analysis date inside)
═══════════════════════════════════════════════
